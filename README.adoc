...
Paul Tomei, Liam Rigby et Alexandre Fouchard
...
Les 4 couches qui structurent un code sont : Présentation=>Service=>Domaine=>Infrastructure. Chaque couche dépend de sa couche n+1.
Dans le code de cet exercice, il n'y a pas de couches car le code n'est pas structuré en package.

Ce code peut poser problème car il est moins sécurisé et peut être difficile à maintenir

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*

Le code métier se situe dans le TodoListController. 

*Cette règle est-elle facilement testable par un test unitaire ?*

Elle n'est pas testable car elle est iumbriqué dans le controlleur, elle n'est pas testable par un test unitaire.
...
On demande à docker de lancer une image docker avec le mdp "password" sur le port "5432:5432" et en postgres.
...
Elle se base sur le principe d'inversion des dépendances qui permet de créer des programmes flexibles et stables.
Elle permet aussi de séparer la logique métier avec les briques technologiques (API, bases de données,...).
Cette architecture permet de favoriser les tests en isolation.
...
Nous testons si le système réagit 
